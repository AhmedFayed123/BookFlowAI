...
const emptyService = (): UpdateServiceDto => ({ businessCategoryId: 0, name: "", description: "", price: 0, durationInMinutes: 30, isActive: true });
...
export default function CatalogPage() {
  ...
  const saveService = async (event: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      if (serviceId !== null) await servicesApi.update(serviceId, service);
      else await servicesApi.create({ name: service.name, description: service.description, businessCategoryId: service.businessCategoryId, price: service.price, durationInMinutes: service.durationInMinutes });
