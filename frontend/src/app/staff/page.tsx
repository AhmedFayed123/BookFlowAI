          ...
          <section className="grid gap-6 lg:grid-cols-[1fr_1.7fr]">
            <div className="space-y-6">
              <div className="surface-card rounded-2xl p-6 shadow-sm">
                <h2 className="font-semibold text-slate-900">Weekly shifts</h2>
              ...
              <form onSubmit={requestDayOff} className="surface-card rounded-2xl p-6 shadow-sm">
                <h2 className="font-semibold text-slate-900">Request time off</h2>
                ...
                <button type="submit" className="mt-3 w-full rounded-xl button-primary px-4 py-3 font-semibold text-white">Submit request</button>
              </form>
            </div>

            <div className="surface-card rounded-2xl p-6 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="font-semibold text-slate-900">Assigned bookings</h2>
              ...
              <div className="mt-5 space-y-3">
                {loading ? <AsyncState loading /> : bookings.length === 0 ? <p className="text-slate-500">No bookings for this date.</p> : bookings.map((booking) => (
                  <article key={booking.id} className="rounded-xl border border-slate-200 p-4">
                    <div className="flex flex-wrap justify-between gap-2"><div><h3 className="font-semibold text-slate-900"><Link href={`/bookings/${booking.id}`} className="hover:text-emerald-800">{booking.serviceName}</Link></h3>
                      ...
                      <span className={`h-fit rounded-full px-3 py-1 text-xs font-semibold ${statusStyle[booking.status] ?? "bg-slate-100 text-slate-700"}`}>{booking.status}</span>
                    </div>
                    {(booking.status === "Pending" || booking.status === "Confirmed") && <div className="mt-4 flex flex-wrap gap-2">
                      {booking.status === "Pending" && <button type="button" onClick={() => void updateStatus(booking.id, "confirm")} className="rounded-lg bg-slate-700 px-3 py-2 text-sm font-semibold text-white">Confirm</button>}
